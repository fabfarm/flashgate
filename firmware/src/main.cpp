...
void setup() {
  ...
  Serial.println("Touch button (GPIO14) ready - tap to open gate and test servo!");
}
...
void loop() {
  ...
  if (gateOpenedByTouch && (millis() - gateOpenTime_touch > gateOpenTime)) {
    Serial.println("Closing gate after touch test...");
    closeGate();
    gateOpenedByTouch = false;
  }
  
  // Normal pattern detection (only if gate is not currently open from touch)
  if (!gateOpenedByTouch && detectPattern()) {
